...
extern void init_mac(const uint8_t *mymac); // not needed if you call init_udp_or_www_server
...
// there is no answer. 
extern void get_mac_with_arp(uint8_t *ip,
...
extern void client_browse_url(const char *urlbuf_p,
							  char *urlbuf_varpart,
							  const char *hoststr,
...
extern void client_http_post(const char *urlbuf_p,
							 char *urlbuf_varpart,
							 const char *hoststr,
...
extern void send_udp(uint8_t *buf,
					 char *data,
					 uint8_t datalen,
					 uint16_t sport,
					 const uint8_t *dip,
