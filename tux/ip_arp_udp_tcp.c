...
#define status_led_off PORTB|=(1<<PORTB1)
#define status_led_on PORTB&=~(1<<PORTB1)
...
static void (*client_browser_callback)(uint16_t,
									   uint16_t,
									   uint16_t); // the fields are: uint16_t webstatuscode,uint16_t datapos,uint16_t len; datapos is start of http data and len the the length of that data
...
static uint8_t ipid =
	0x2; // IP-identification, it works as well if you do not change it but it is better to fill the field, we count this number up and wrap.
const char iphdr[] PROGMEM = {0x45, 0, 0, 0x82, 0, 0, 0x40, 0,
							  0x20}; // 0x82 is the total len on ip, 0x20 is ttl (time to live), the second 0,0 is IP-identification and may be changed.
#endif
...
void init_mac(const uint8_t *mymac) {
  if (mymac) {
...
// init_udp_or_www_server or client_ifconfig 
// before you can use this function
void get_mac_with_arp(uint8_t *ip,
					  uint8_t reference_number,
					  void (*arp_result_callback)(uint8_t *ip, uint8_t reference_number, uint8_t *mac)) {
  uint8_t i = 0;
...
// limited in memory
//
uint8_t client_tcp_req(uint8_t (*result_callback)(uint8_t fd,
												  uint8_t statuscode,
...
//	  len = fill_tcp_data_p(bufptr, len, PSTR("\r\nX-Auth-Token: BBFF-iEBkIk2W4d0EcCA39WkF1qfeYDOm6N"));
//                        len=fill_tcp_data_p(bufptr,len,PSTR("\r\nUser-Agent: tgr/1.1\r\nAccept: */*\r\n"));
	  len = fill_tcp_data_p(bufptr, len, PSTR("\r\nContent-Length: "));
...
#ifdef ENC28J60_DEBUG_REQUEST
	printf_P(PSTR("FQ:\n"));
	puts((char*)(&bufptr[TCP_CHECKSUM_L_P + 3]));
	printf_P(PSTR("\n<\n"));
#endif
	return (len);
  }
...
uint8_t www_client_internal_result_callback(uint8_t fd, uint8_t statuscode, uint16_t datapos, uint16_t len_of_data) {
  ...
  if (statuscode == 0 && len_of_data > 12) {
	...
	if (client_browser_callback) {
	  if (isblank(bufptr[datapos + 8]) && isdigit(bufptr[datapos + 9]) && isdigit(bufptr[datapos
		  + 11])) { // e.g 200 OK, a status code has 3 digits from datapos+9 to datapos+11, copy over the web/http status code to web_statuscode:
		while (i < 2) {
...
//
void client_browse_url(const char *urlbuf_p,
					   char *urlbuf_varpart,
					   const char *hoststr,
					   void (*callback)(uint16_t, uint16_t, uint16_t),
					   uint8_t *dstip,
					   uint8_t *dstmac) {
  if (!enc28j60linkup())return;
  ...
  www_fd =
	  client_tcp_req(&www_client_internal_result_callback, &www_client_internal_datafill_callback, 80, dstip, dstmac);
}
...
void client_http_post(const char *urlbuf_p,
					  char *urlbuf_varpart,
					  const char *hoststr,
					  const char *additionalheaderline_p,
					  char *postval,
					  void (*callback)(uint16_t, uint16_t, uint16_t),
					  uint8_t *dstip,
					  uint8_t *dstmac) {
...
static uint8_t mqtt_fd = 0;

static uint8_t mqtt_connected_flag = 0;

static void (*mqtt_callback)(uint16_t,
							 uint16_t,
							 uint16_t); // the fields are: uint16_t mqttstatuscode,uint16_t datapos,uint16_t len; datapos is start of http data and len the the length of that data

uint8_t mqtt_connected(void){
  return (mqtt_connected_flag);
}

uint16_t mqtt_client_internal_datafill_callback(uint8_t fd) {
//  char strbuf[5];
  uint16_t len = TCP_CHECKSUM_L_P + 3;
  if (fd == mqtt_fd) {
...
//	  len = fill_tcp_data(bufptr, 0, 0x10);
//	  len = fill_tcp_data_p(bufptr, len, client_urlbuf_p);
//	  len = fill_tcp_data(bufptr, len, client_urlbuf_var);

#ifndef ENC28J60_DEBUG_REQUEST
	for (uint16_t kI = TCP_CHECKSUM_L_P + 3; kI < len; ++kI) {
	  uint8_t c = bufptr[kI];
	  printf("%02X ", c);
	}
	puts("");
	...
	uint16_t size = (len - TCP_CHECKSUM_L_P - 3);
...
uint8_t mqtt_client_internal_result_callback(uint8_t fd, uint8_t statuscode, uint16_t datapos, uint16_t len_of_data) {
  if (fd != mqtt_fd) {
	mqtt_connected_flag = 0;
	printf_P(PSTR("ERROR FD = %02X\n"), fd);
	(*mqtt_callback)(500, 0, 0);
	return (0);
  }
  printf_P(PSTR("\nSTATUS FD = %02X\n"), statuscode);
  if (statuscode == 0) {
	...
	if (mqtt_callback) {
//	  if (isblank(bufptr[datapos])) {
//
//	  }
	  //(*client_browser_callback)(web_statuscode,((uint16_t)TCP_SRC_PORT_H_P+(bufptr[TCP_HEADER_LEN_P]>>4)*4),len_of_data);
	  (*mqtt_callback)(statuscode, datapos, len_of_data);
	}
	return (1); //(0) mean hold connection
//	  return (0); //(1) mean close connection
...
/**
 * @brief MQTT request
 * @details This maintain TCP connection
 * @param result_callback
 * @param datafill_callback
 * @param port
 * @param dstip
 * @param dstmac
 * @return
 */
uint8_t mqtt_tcp_req(uint8_t (*result_callback)(uint8_t fd,
												  uint8_t statuscode,
												  uint16_t data_start_pos_in_buf,
												  uint16_t len_of_data),
					   uint16_t (*datafill_callback)(uint8_t fd),
  ...
  tcp_client_port = port;
  tcp_client_state = state; // send a syn
  tcp_fd++;
  if (tcp_fd > 7) {
	tcp_fd = 0;
  }
...
void mqtt_connect(void (*callback)(unsigned int, unsigned int, unsigned int),
				  uint8_t *dstip,
				  uint8_t *dstmac) {
  if (!enc28j60linkup())return;
  ...
  if (!mqtt_connected_flag) {
    puts_P(PSTR("NOT CONNECTED"));
	mqtt_fd = mqtt_tcp_req(&mqtt_client_internal_result_callback,
						   &mqtt_client_internal_datafill_callback,
						   1883,
						   dstip,
						   dstmac,
						   1);
  } else{
    puts_P(PSTR("CONNECTED"));
	mqtt_fd = mqtt_tcp_req(&mqtt_client_internal_result_callback,
						   &mqtt_client_internal_datafill_callback,
						   1883,
						   dstip,
